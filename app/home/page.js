...
function SectionHeader({ label, sub, cta, onCta }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
      ...
      {cta && (
        <button onClick={onCta} style={{ fontSize: 12, fontWeight: 800, color: INDIGO, background: SOFT, border: 'none', borderRadius: 20, padding: '6px 14px', cursor: 'pointer', fontFamily: FONT, whiteSpace: 'nowrap' }}>
          {cta} →
        </button>
      )}
    </div>
...
export default function Home() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: BG, fontFamily: FONT, direction: isRtl ? 'rtl' : 'ltr', paddingBottom: 90 }}>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 20px 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 28 }}>
          {t.stages.map((stage, i) => (
            <button key={i} onClick={() => router.push(stage.route)} style={{
        ...
        {/* Ask */}
        <SectionHeader label={t.askTitle} sub={t.askSub} />
        <button onClick={() => router.push('/reber/ask')} style={{
        ...
        {loading ? (
          ...
          listings.map(listing => {
            ...
            const title = isService ? d.serviceName : d.jobTitle
            const sub = isService ? (d.price || '') : (d.salary ? `💰 ${d.salary}` : '')
              ...
              <div key={listing.id} style={{
                ...
                <div style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {d.location && <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>📍 {d.location}</div>}
                  </div>
                  {d.contact && (
                    <a href={d.contact.includes('@') ? `mailto:${d.contact}` : `tel:${d.contact.replace(/\s/g,'')}`}
                      onClick={e => e.stopPropagation()}
                      style={{ background: `linear-gradient(135deg, ${INDIGO}, ${INDIGO_LIGHT})`, borderRadius: 10, padding: '7px 12px', color: '#fff', fontWeight: 700, fontSize: 12, textDecoration: 'none', flexShrink: 0 }}>
                      📞
                    </a>
                  )}
                </div>
