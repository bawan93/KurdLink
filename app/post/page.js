...
function PostInner() {
  ...
  const checkAuth = async () => {
    ...
    setLoading(false)
    // Don't redirect — we show sign up prompt if not logged in
  }
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '24px 20px', boxSizing: 'border-box' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 14, color: '#888', margin: 0, lineHeight: 1.5 }}>
            {lang === 'en' ? 'Select a category to get started. You\'ll need an account to post.' : 'پۆلێک هەڵبژێرە بۆ دەستپێکردن. پێویستت بە ئەکاونت هەیە بۆ پۆستکردن.'}
          </p>
        </div>

        {/* Sign up notice */}
        <div style={{ background: 'rgba(255,107,53,0.08)', border: '1.5px solid rgba(255,107,53,0.2)', borderRadius: 14, padding: '14px 16px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 20 }}>🔒</span>
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: '#FF6B35', margin: '0 0 2px' }}>
              {lang === 'en' ? 'Account required to post' : 'ئەکاونت پێویستە بۆ پۆستکردن'}
            </p>
            <p style={{ fontSize: 12, color: '#888', margin: 0 }}>
              {lang === 'en' ? 'Free to sign up — takes 30 seconds' : 'خۆڕایییە — ٣٠ چرکە دەخایەنێت'}
            </p>
          </div>
          <button onClick={() => router.push('/onboarding')} style={{ background: ORANGE, border: 'none', borderRadius: 10, padding: '8px 14px', color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer', fontFamily: FONT, flexShrink: 0 }}>
            {lang === 'en' ? 'Sign Up' : 'تۆمارکردن'}
          </button>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {OPTIONS.map(opt => (
            <button
              ...
              onClick={() => router.push(`/onboarding?redirect=/listings/${opt.id}`)}
              style={{
                background: '#fff',
            ...
            >
              ...
              <span style={{ color: 'rgba(0,0,0,0.2)', fontSize: 20, flexShrink: 0, transform: isRtl ? 'rotate(180deg)' : 'none' }}>›</span>
            </button>
          ))}
        </div>

        {/* Already have account */}
        <div style={{ textAlign: 'center', marginTop: 28, paddingTop: 20, borderTop: '1px solid rgba(0,0,0,0.07)' }}>
          <span style={{ fontSize: 13, color: '#888' }}>{lang === 'en' ? 'Already have an account? ' : 'ئەکاونتی هەیت؟ '}</span>
          <span onClick={() => router.push('/auth')} style={{ fontSize: 13, fontWeight: 700, color: '#FF6B35', cursor: 'pointer' }}>
