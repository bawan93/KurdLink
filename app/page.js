  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', minHeight: '100vh', background: '#F5F5F7', fontFamily: isRtl ? "'Noto Sans Arabic', sans-serif" : "'DM Sans', sans-serif", direction: t.dir, position: 'relative' }}>
      ...
      <div style={{ padding: '20px 16px 80px', overflowY: 'auto' }}>
        ...
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: '22px', fontWeight: 800, color: '#1C1C1E', lineHeight: 1.25, marginBottom: '18px' }}>{t.hero[0]}<br />{t.hero[1]}</h1>

        {/* Search */}
        <div style={{ background: '#fff', borderRadius: '14px', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '8px', boxShadow: '0 2px 12px rgba(0,0,0,.07)', marginBottom: '24px', border: '2px solid transparent', transition: 'border-color .2s' }} onFocus={e => e.currentTarget.style.borderColor = '#FF6B35'} onBlur={e => e.currentTarget.style.borderColor = 'transparent'}>
          <span style={{ fontSize: '16px', color: '#AEAEB2' }}>🔍</span>
          <input placeholder={t.searchPh} style={{ flex: 1, border: 'none', outline: 'none', fontSize: '14px', color: '#1C1C1E', background: 'transparent', direction: t.dir, fontFamily: 'inherit' }} />
        </div>

        {/* Categories */}
        <div style={{ marginBottom: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '13px' }}>
            <span style={{ fontFamily: 'Syne, sans-serif', fontSize: '18px', fontWeight: 800, color: '#1C1C1E' }}>{t.categories}</span>
            <span style={{ fontSize: '12px', fontWeight: 600, color: '#FF6B35', cursor: 'pointer' }}>{t.seeAll}</span>
          </div>
          <div style={{ display: 'flex', overflowX: 'auto', gap: '12px', paddingBottom: '4px', scrollbarWidth: 'none' }}>
            {CATEGORIES.map((cat, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px', flexShrink: 0, cursor: 'pointer', minWidth: '54px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '13px' }}>
            <span style={{ fontFamily: 'Syne, sans-serif', fontSize: '18px', fontWeight: 800, color: '#1C1C1E' }}>{t.featured}</span>
            <span style={{ fontSize: '12px', fontWeight: 600, color: '#FF6B35', cursor: 'pointer' }}>{t.seeAll}</span>
          </div>
          <div style={{ display: 'flex', overflowX: 'auto', gap: '13px', paddingBottom: '8px', scrollbarWidth: 'none' }}>
            {BUSINESSES.slice(0, 3).map(b => (
              <div key={b.id} style={{ flexShrink: 0, width: '265px', background: '#fff', borderRadius: '20px', padding: '17px', position: 'relative', boxShadow: '0 4px 20px rgba(0,0,0,.08)', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', gap: '9px', alignItems: 'center', marginBottom: '10px' }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: '13px', color: '#1C1C1E', marginBottom: '2px' }}>{b.name[lang]}</div>
        ...
        <div style={{ marginBottom: '22px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '13px' }}>
            <span style={{ fontFamily: 'Syne, sans-serif', fontSize: '18px', fontWeight: 800, color: '#1C1C1E' }}>{t.nearby}</span>
            <span style={{ fontSize: '12px', fontWeight: 600, color: '#FF6B35', cursor: 'pointer' }}>{t.seeAll}</span>
          </div>
          {BUSINESSES.slice(3, 5).map(b => (
            <div key={b.id} style={{ background: '#fff', borderRadius: '16px', padding: '13px', display: 'flex', gap: '11px', alignItems: 'center', boxShadow: '0 2px 10px rgba(0,0,0,.05)', marginBottom: '10px', cursor: 'pointer' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: '13px', color: '#1C1C1E', marginBottom: '1px' }}>{b.name[lang]}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '6px', flexShrink: 0 }}>
                ...
                <button onClick={() => toggleSave(b.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: saved.includes(b.id) ? '#FF6B35' : '#AEAEB2' }}>{saved.includes(b.id) ? '♥' : '♡'}</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ background: 'linear-gradient(135deg,#1C1C2E,#2D1B4E)', borderRadius: '19px', padding: '22px', position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ fontSize: '16px', fontWeight: 800, color: '#fff', fontFamily: 'Syne, sans-serif', lineHeight: 1.3, marginBottom: '6px' }}>{t.bizCtaTitle}</p>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: '480px', background: 'rgba(255,255,255,.97)', borderTop: '1px solid rgba(0,0,0,.07)', display: 'flex', zIndex: 50 }}>
        {[
          ...
          <button key={item.id} onClick={() => setTab(item.id)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', background: 'none', border: 'none', cursor: 'pointer', padding: '8px 0', fontFamily: 'inherit' }}>
