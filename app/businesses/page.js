...
export default function Businesses() {
  ...
  const T = {
    en: { title: 'Businesses for Sale', subtitle: 'Browse businesses listed by the Kurdish community', empty: 'No businesses listed yet', emptyDesc: 'Check back soon or list your business', listBiz: 'Sell Your Business' },
    ku: { title: 'بیزنس بۆ فرۆشتن', subtitle: 'بیزنسەکانی لیستکراو لە کۆمەڵگای کورد', empty: 'هێشتا هیچ بیزنسێک لیست نەکراوە', emptyDesc: 'دواتر سەردان بکەرەوە یان بیزنسەکەت لیست بکە', listBiz: 'بیزنسەکەت بفرۆشە' }
  }
  const t = T[lang]
  ...
  const fetchBusinesses = async () => {
    ...
    const { data } = await supabase.from('listings').select('*').eq('type', 'sell_business').eq('status', 'approved').order('created_at', { ascending: false })
    setListings(data || [])
    setLoading(false)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f7f5', fontFamily: FONT, direction: isRtl ? 'rtl' : 'ltr', paddingBottom: 80 }}>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '16px 16px 20px', boxSizing: 'border-box' }}>
        <button onClick={() => router.push('/listings/sell-business')} style={{ width: '100%', background: ORANGE, border: 'none', borderRadius: 14, padding: '14px', color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: FONT, marginBottom: 16, boxShadow: '0 4px 16px rgba(255,107,53,0.3)' }}>
          + {t.listBiz}
        </button>
          ...
          listings.map(listing => {
            ...
            return (
              <div key={listing.id} style={{ background: '#fff', borderRadius: 16, marginBottom: 12, border: '1.5px solid rgba(0,0,0,0.06)', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
                {img && <img src={img} alt="business" style={{ width: '100%', height: 160, objectFit: 'cover' }} />}
                <div style={{ padding: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                    <h3 style={{ fontSize: 17, fontWeight: 800, color: NAVY, margin: 0, flex: 1, paddingRight: 8 }}>{d.businessName}</h3>
                    <span style={{ fontSize: 17, fontWeight: 800, color: '#FF6B35', flexShrink: 0 }}>£{Number(d.askingPrice).toLocaleString()}</span>
                  </div>
