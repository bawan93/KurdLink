...
const TX = {
  en: {
    heroTitle: 'Find Jobs & Services',
    heroSub: 'Browse opportunities from the Kurdish community',
    jobs: 'Jobs',
    services: 'Services',
    location: 'Location',
    salary: 'Pay',
  ...
  ku: {
    ...
    heroSub: 'دەرفەتەکان لە کۆمەڵگای کوردی گەڕان',
    jobs: 'کارەکان',
    ...
    price: 'نرخ',
    contact: 'پەیوەندی',
    empty: 'هیچ نییە هێشتا.',
    paused: 'وەستاوە',
    filled: 'پڕکراوە',
  },
  fa: {
    heroTitle: 'شغل و خدمات پیدا کن',
    heroSub: 'فرصت‌های جامعه کردی را مرور کن',
    jobs: 'مشاغل',
    services: 'خدمات',
    location: 'موقعیت',
    salary: 'حقوق',
    price: 'نرخ',
    contact: 'تماس',
    empty: 'هنوز چیزی نیست.',
    paused: 'متوقف',
    filled: 'پر شده',
  },
  ar: {
    heroTitle: 'ابحث عن وظائف وخدمات',
    heroSub: 'تصفح الفرص من المجتمع الكردي',
    jobs: 'الوظائف',
    services: 'الخدمات',
    location: 'الموقع',
    salary: 'الراتب',
                ...
                <div key={listing.id} style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 12px rgba(79,70,229,0.07)', overflow: 'hidden', opacity: isFilled ? 0.7 : 1 }}>
                  ...
                  <div style={{ background: tab === 'jobs' ? SOFT : '#F0FDF4', padding: '6px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 800, color: tab === 'jobs' ? INDIGO : '#059669', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                        {tab === 'jobs' ? t.jobs.replace(/s$/, '') : t.services.replace(/s$/, '')}
                      </span>
                    </div>
                    {isFilled && (
                      <span style={{ background: '#FEF3C7', color: '#D97706', fontSize: 10, fontWeight: 800, padding: '2px 8px', borderRadius: 20 }}>{t.filled}</span>
                    )}
                  </div>
                  ...
                  <div style={{ padding: '14px 16px' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                      {d.location && (
                        <span style={{ background: BG, color: INDIGO_DARK, fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 20 }}>
                          📍 {d.location}
                        </span>
                      )}
                      {(d.salary || d.price) && (
                        <span style={{ background: '#F0FDF4', color: '#059669', fontSize: 12, fontWeight: 600, padding: '4px 10px', borderRadius: 20 }}>
                          💰 {d.salary || d.price}
                        </span>
                      )}
                    </div>
                    ...
                    {d.contact && !isFilled && (
                      <a
                        href={d.contact.includes('@') ? `mailto:${d.contact}` : `tel:${d.contact.replace(/\s/g, '')}`}
