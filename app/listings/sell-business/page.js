...
function SellBusinessInner() {
  ...
  const [form, setForm] = useState({
    businessName: '', city: '', postcode: '', askingPrice: '',
    description: '', phone: '', email: ''
  })

  const isRtl = lang === 'ku'
  const T = {
    en: { title: 'Sell Your Business', subtitle: 'Fill in the details and we\'ll review your listing within 24 hours', businessName: 'Business Name', businessNamePh: 'e.g. Ahmad & Co Grocers', city: 'City', cityPh: 'e.g. Leeds', postcode: 'Postcode', postcodePh: 'e.g. LS1 4AP', askingPrice: 'Asking Price (£)', askingPricePh: 'e.g. 50000', description: 'About the Business', descriptionPh: 'Describe the business, why you\'re selling, what\'s included…', phone: 'Phone Number', phonePh: '+44 7700 900000', email: 'Email Address', emailPh: 'you@example.com', images: 'Business Photos', imagesDesc: 'Upload up to 5 photos of your business', submit: 'Submit for Review', submitting: 'Submitting…', back: '← Back', successTitle: 'Submitted! 🎉', successMsg: 'Your listing is under review. We\'ll notify you within 24 hours.', backHome: 'Back to Home', errFill: 'Please fill in all required fields' },
    ku: { title: 'بیزنسەکەت بفرۆشە', subtitle: 'زانیاریەکان پڕ بکەوە، ئێمەش لە ٢٤ کاتژمێردا پێداچوونەوەی دەکەین', businessName: 'ناوی بیزنس', businessNamePh: 'وەک: دوکانی ئەحمەد', city: 'شار', cityPh: 'وەک: لیدز', postcode: 'پۆستکۆد', postcodePh: 'وەک: LS1 4AP', askingPrice: 'نرخی داواکراو (£)', askingPricePh: 'وەک: ٥٠٠٠٠', description: 'دەربارەی بیزنس', descriptionPh: 'بیزنسەکە وەسف بکە…', phone: 'ژمارەی تەلەفۆن', phonePh: '+44 7700 900000', email: 'ئیمەیڵ', emailPh: 'you@example.com', images: 'وێنەی بیزنس', imagesDesc: 'تا ٥ وێنە بار بکە', submit: 'بنێرە بۆ پێداچوونەوە', submitting: 'دەنێردرێت…', back: '→ گەڕانەوە', successTitle: '!دانرا 🎉', successMsg: 'داواکاریەکەت تەماشا دەکرێت. لە ٢٤ کاتژمێردا ئاگادارت دەکەینەوە.', backHome: 'گەڕانەوە بۆ ماڵەوە', errFill: 'تکایە هەموو خانەکان پڕ بکەوە' }
  }
  const t = T[lang]
  ...
  const handleSubmit = async () => {
    setError('')
    const { businessName, city, postcode, askingPrice, description, phone, email } = form
    if (!businessName || !city || !postcode || !askingPrice || !description || !phone || !email) return setError(t.errFill)
    setLoading(true)
    try {
      ...
      const { error: e } = await supabase.from('listings').insert({ type: 'sell_business', user_id: user?.id, status: 'pending', data: { ...form, imageUrls } })
      if (e) throw e
      setSubmitted(true)
          ...
          <div>
            ...
            {images.length > 0 && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 8 }}>
                {images.map((img, i) => (
                  <div key={i} style={{ fontSize: 12, background: '#e8f5e9', color: '#2e7d32', padding: '4px 10px', borderRadius: 20, fontWeight: 600 }}>✓ {img.name.slice(0, 15)}…</div>
                ))}
              </div>
            )}
          </div>
