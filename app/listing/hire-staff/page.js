...
export default function HireStaffPage() {
  ...
  const t = (TX[lang] || TX.en).hireStaff
  ...
  async function handleSubmit() {
    ...
    if (!jobTitle || !description || !location || !salary || !contact) { setError(t.required); return }
    setError(''); setSubmitting(true)
    const { error: sbError } = await supabase.from('listings').insert({ type: 'hire_staff', status: 'pending', user_id: userId, data: { jobTitle, description, location, salary, contact } })
    setSubmitting(false)
